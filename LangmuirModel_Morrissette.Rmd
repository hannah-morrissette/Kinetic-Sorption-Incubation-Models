...
#   geom_line(aes(time, DOC), color = "darkseagreen", size = 1.2) +
#   geom_point(data = dat4, aes(time, DOC)) +
#   theme_minimal() +
...
#   geom_point(data = dat4, aes(time, DOC)) +
#   theme_minimal() +
#   labs(x = "Time (hrs)", y = "DOC (mg)", title = "Model and Observed DOC over Time", subtitle = site, tag = tag) +
#   annotate("text",x = 12, y = ylabel, label = st)
# #annotate is better than geom_text - puts as many texts on as points

# ggsave(filename=paste(site,"tag",".png",sep=""), width = 7.5, height = 7.5, units = "in")
# ggsave(filename=paste(site,"tag",".tiff",sep=""), width = 7.5, height = 7.5, units = "in")
