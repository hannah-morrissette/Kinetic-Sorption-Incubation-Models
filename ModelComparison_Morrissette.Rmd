...
line_vals <- read.csv("iso_graph_lines.csv")
...
jbf <- data.frame(exp = rep("JBF",320)) %>% mutate(
                  DOCf = 0:319,
                  dDOC = dDOC.jbf,
                  ci.min = dDOC.jbf-me.jbf,
                  ci.max = dDOC.jbf+me.jbf,
                  se.min = dDOC.jbf-se.jbf,
                  se.max = dDOC.jbf+se.jbf)
...
jbs <- data.frame(exp = rep("JBS",320)) %>% mutate(
                  DOCf = 0:319,
                  dDOC = dDOC.jbs,
                  ci.min = dDOC.jbs-me.jbs,
                  ci.max = dDOC.jbs+me.jbs,
                  se.min = dDOC.jbs - se.jbs,
                  se.max = dDOC.jbs + se.jbs)
#TAF
...
taf <- data.frame(exp = rep("TAF",320)) %>% mutate(
                  DOCf = 0:319,
                  dDOC = dDOC.taf,
                  ci.min = dDOC.taf-me.taf,
                  ci.max = dDOC.taf+me.taf,
                  se.min = dDOC.taf - se.taf,
                  se.max = dDOC.taf + se.taf)
#TAS
...
tas <- data.frame(exp = rep("TAS",320)) %>% mutate(
                  DOCf = 0:319,
                  dDOC = dDOC.tas,
                  ci.min = dDOC.tas-me.tas,
                  ci.max = dDOC.tas+me.tas,
                  se.min = dDOC.tas - se.tas,
                  se.max = dDOC.tas + se.tas)

lang.data <- rbind(jbf,jbs,taf,tas)

lang.iso <- ggplot(line_vals) +
  geom_line(aes(DOCf, dDOC, color = exp, lty = exp), size = 1.3) +
  geom_hline(yintercept = 0, color = "black") +
  geom_ribbon(data=line_vals, aes(x=DOCf, ymin=ymin, ymax=ymax, color = exp, fill = exp), alpha = 0.2) +
  geom_errorbar(data=kvals, aes(x=DOCf.mg.L, ymin=ymin, ymax=ymax), width = 4) +
  geom_point(data=kvals, aes(DOCf.mg.L, dDOC.mg.g, color = exp, shape = exp), size = 4.5) +
